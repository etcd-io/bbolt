...
func (tx *Tx) Commit() error {
	Assert(!tx.managed, "managed tx commit not allowed")
	if tx.db == nil {
		return ErrTxClosed
	} else if !tx.writable {
	...
	if !tx.db.NoFreelistSync {
		...
		tx.meta.SetFreelist(PgidNoFreelist)
	}
...
func (tx *Tx) Rollback() error {
	Assert(!tx.managed, "managed tx rollback not allowed")
	if tx.db == nil {
		return ErrTxClosed
	}
