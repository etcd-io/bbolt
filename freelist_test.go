...
func TestFreelist_release(t *testing.T) {
	f := newTestFreelist()
	f.free(100, NewPage(12, 0, 0, 1))
	f.free(100, NewPage(9, 0, 0, 0))
	f.free(102, NewPage(39, 0, 0, 0))
	f.release(100)
	f.release(101)
...
func TestFreelist_releaseRange(t *testing.T) {
	...
	for _, c := range releaseRangeTests {
		...
		for _, p := range c.pagesIn {
			f.free(p.freeTxn, NewPage(p.id, 0, 0, uint32(p.n-1)))
		}
