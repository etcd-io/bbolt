...
// newTypedFreelist get the freelist type from env and initial the freelist
func newTypedFreelist() *freelist {
	if env := os.Getenv(arrayFreelist); env != "" {
		return newFreelist(ArrayType)
	}

	return newFreelist(HashMapType)
}
