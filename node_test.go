...
func TestNode_read_LeafPage(t *testing.T) {
	...
	nodes[0] = *NewLeafPageElement(0, 32, 3, 4)  // pos = sizeof(leafPageElement) * 2
	nodes[1] = *NewLeafPageElement(0, 23, 10, 3) // pos = sizeof(leafPageElement) + 3 + 4

	// Write data for the nodes at the end.
	const s = "barfoozhelloworldbye"
	data := UnsafeByteSlice(unsafe.Pointer(uintptr(unsafe.Pointer(page))+unsafe.Sizeof(*page)+LeafPageElementSize*2), 0, 0, len(s))
	copy(data, s)
