...
func (n *node) put(oldKey, newKey, value []byte, pgId Pgid, flags uint32) {
	...
	inode.value = value
	inode.pgid = pgId
	Assert(len(inode.key) > 0, "put: zero-length inode key")
}
...
func (n *node) read(p *Page) {
	...
	if len(n.inodes) > 0 {
		...
		Assert(len(n.key) > 0, "read: zero-length node key")
	} else {
		n.key = nil
	}
}
...
func (n *node) write(p *Page) {
	Assert(p.Count() == 0 && p.Flags() == 0, "node cannot be written into a not empty page")
...
func (n *node) spill() error {
	...
	for _, node := range nodes {
		...
		if node.parent != nil {
			...
			Assert(len(node.key) > 0, "spill: zero-length node key")
		}
...
func (n *node) rebalance() {
	...
	Assert(n.parent.numChildren() > 1, "parent must have at least 2 children")
...
func (n *node) dereference() {
	if n.key != nil {
		...
		copy(key, n.key)
		n.key = key
		Assert(n.pgid == 0 || len(n.key) > 0, "dereference: zero-length node key on existing node")
	}
	...
	for i := range n.inodes {
		...
		copy(key, inode.key)
		inode.key = key
		Assert(len(inode.key) > 0, "dereference: zero-length inode key")
