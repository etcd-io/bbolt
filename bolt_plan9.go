...
func flock(db *DB, exclusive bool, timeout time.Duration) error {
	...
	var flags uint32 = plan9.LOCKFILE_FAIL_IMMEDIATELY
	if exclusive {
		flags |= plan9.LOCKFILE_EXCLUSIVE_LOCK
	}
	for {
		// Fix for https://github.com/etcd-io/bbolt/issues/121. Use byte-range
		// -1..0 as the lock on the database file.
		var m1 uint32 = (1 << 32) - 1 // -1 in a uint32
		err := plan9.LockFileEx(plan9.Handle(db.file.Fd()), flags, 0, 1, 0, &plan9.Overlapped{
			Offset:     m1,
			OffsetHigh: m1,
		})
		...
		if err == nil {
			return nil
		} else if err != plan9.ERROR_LOCK_VIOLATION {
			return err
		}
...
func funlock(db *DB) error {
	var m1 uint32 = (1 << 32) - 1 // -1 in a uint32
	return plan9.UnlockFileEx(plan9.Handle(db.file.Fd()), 0, 1, 0, &plan9.Overlapped{
		Offset:     m1,
		OffsetHigh: m1,
	})
}
