...
func TestOpen_ErrInvalid(t *testing.T) {
	...
	if _, err := bolt.Open(path, 0666, nil); err != ErrInvalid {
...
func TestOpen_ErrVersionMismatch(t *testing.T) {
	...
	// Reopen data file.
	if _, err := bolt.Open(path, 0666, nil); err != ErrVersionMismatch {
		t.Fatalf("unexpected error: %s", err)
	}
}
...
func TestOpen_ErrChecksum(t *testing.T) {
	...
	// Reopen data file.
	if _, err := bolt.Open(path, 0666, nil); err != ErrChecksum {
		t.Fatalf("unexpected error: %s", err)
	}
}
...
func TestDB_Open_ReadOnly(t *testing.T) {
	...
	if _, err := readOnlyDB.Begin(true); err != ErrDatabaseReadOnly {
		t.Fatalf("unexpected error: %s", err)
	}
