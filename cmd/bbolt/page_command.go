...
func newPageCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "print page(s) of the bbolt database",
		Long:  "print one or more page(s) of the  bbolt database.",
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) < 2 {
				return fmt.Errorf("requires at least 2 args, got %d\nUsage: bbolt page <bbolt-file> <page-id>", len(args))
			}
			// Extract arguments by name
			path, pageIDs := args[0], args[1:]

			// Call pageFunc with the destructured arguments
			return pageFunc(cmd, path, pageIDs)
		},
	}
...
// pageFunc contains the logic that was originally in RunE
func pageFunc(cmd *cobra.Command, path string, pageIDs []string) error {
	...
	parsedPageIDs, err := stringToPages(pageIDs)
	if err != nil {
		return err
	} else if len(parsedPageIDs) == 0 {
		return fmt.Errorf("page ID(s) required")
	}

	return printPages(parsedPageIDs, path, formatValue)
}
